...
func (h *SnapshotsHandler) PreviewRestore(c *gin.Context) {
	...
	// Dispatch a restore preview command to the agent via the command queue.
	payload := &models.CommandPayload{
		SnapshotID:   req.SnapshotID,
		RepositoryID: req.RepositoryID,
		TargetPath:   req.TargetPath,
	}
	cmd := models.NewAgentCommand(agentID, user.CurrentOrgID, models.CommandTypeRestorePreview, payload, &user.ID)
	cmd.CreatedByName = user.Name

	if err := h.store.CreateAgentCommand(c.Request.Context(), cmd); err != nil {
		h.logger.Error().Err(err).Msg("failed to create restore preview command")
...
func (h *SnapshotsHandler) DiffFile(c *gin.Context) {
	...
	// Dispatch a file diff command to the agent via the command queue.
	payload := &models.CommandPayload{
		SnapshotID:  snapshotID1,
		SnapshotID2: snapshotID2,
		FilePath:    filePath,
	}
	cmd := models.NewAgentCommand(backup1.AgentID, user.CurrentOrgID, models.CommandTypeFileDiff, payload, &user.ID)
	cmd.CreatedByName = user.Name

	if err := h.store.CreateAgentCommand(c.Request.Context(), cmd); err != nil {
		h.logger.Error().Err(err).Msg("failed to create file diff command")
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initiate file diff"})
		return
	}
	...
	c.JSON(http.StatusAccepted, gin.H{
		...
		"file_path":     filePath,
		"message":       "File diff initiated. Poll the command status for results.",
	})
}
...
func (h *SnapshotsHandler) CompareSnapshots(c *gin.Context) {
	...
	// Dispatch a snapshot diff command to the agent via the command queue.
	payload := &models.CommandPayload{
		SnapshotID:  snapshotID1,
		SnapshotID2: snapshotID2,
	}
	cmd := models.NewAgentCommand(backup1.AgentID, user.CurrentOrgID, models.CommandTypeSnapshotDiff, payload, &user.ID)
	cmd.CreatedByName = user.Name

	if err := h.store.CreateAgentCommand(c.Request.Context(), cmd); err != nil {
		h.logger.Error().Err(err).Msg("failed to create snapshot diff command")
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initiate snapshot comparison"})
		return
	}
	...
	c.JSON(http.StatusAccepted, gin.H{
		...
		"snapshot_id_1": snapshotID1,
		"snapshot_id_2": snapshotID2,
		"snapshot_1":    snapshot1Info,
		"snapshot_2":    snapshot2Info,
		"message":       "Snapshot comparison initiated. Poll the command status for results.",
	})
}
