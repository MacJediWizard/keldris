...
func (d *DockerBackup) ListContainers(ctx context.Context) ([]Container, error) {
	...
	for _, line := range lines {
		...
		// Parse created time
		if rawContainer.CreatedAt != "" {
			// Docker format: "2024-01-15 10:30:45 -0700 MST"
			if t, err := time.Parse("2006-01-02 15:04:05 -0700 MST", rawContainer.CreatedAt); err == nil {
				container.Created = t
			}
		}
...
func (d *DockerBackup) ListVolumes(ctx context.Context) ([]Volume, error) {
	...
	for _, line := range lines {
		...
		if rawVolume.Labels != "" {
			...
			for _, label := range strings.Split(rawVolume.Labels, ",") {
				parts := strings.SplitN(label, "=", 2)
				if len(parts) == 2 {
					volume.Labels[parts[0]] = parts[1]
				}
			}
		}
