...
type StandaloneToastVariant = 'success' | 'error' | 'warning' | 'info';
...
const standaloneVariantStyles: Record<StandaloneToastVariant, string> = {
	success:
...
export function Toast({
	...
	useEffect(() => {
		if (onClose && duration && duration > 0) {
	...
	return (
		<div
			role="alert"
			className={`flex items-center gap-3 p-4 rounded-lg border shadow-lg ${standaloneVariantStyles[variant]}`}
		>
...
// --- Context-based Toast system (ToastProvider / ToastContainer) ---
