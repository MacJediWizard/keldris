...
vi.mock('../../hooks/useBackupScripts', () => ({
	...
	useCreateBackupScript: vi.fn(() => ({
		mutateAsync: mockCreateMutateAsync,
		isPending: false,
	})),
	useUpdateBackupScript: vi.fn(() => ({
		mutateAsync: mockUpdateMutateAsync,
		isPending: false,
	})),
	useDeleteBackupScript: vi.fn(() => ({
...
describe('BackupScriptsEditor', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.mocked(useCreateBackupScript).mockReturnValue({
			mutateAsync: mockCreateMutateAsync,
			isPending: false,
		} as unknown as ReturnType<typeof useCreateBackupScript>);
		vi.mocked(useUpdateBackupScript).mockReturnValue({
			mutateAsync: mockUpdateMutateAsync,
			isPending: false,
		} as unknown as ReturnType<typeof useUpdateBackupScript>);
		vi.mocked(useDeleteBackupScript).mockReturnValue({
			mutateAsync: mockDeleteMutateAsync,
			isPending: false,
		} as unknown as ReturnType<typeof useDeleteBackupScript>);
	});
	...
	it('renders existing scripts as cards', () => {
		...
		expect(screen.getAllByText('Post-Backup (Success)').length).toBeGreaterThan(
			0,
		);
	...
	it('hides fail_on_error checkbox for non pre_backup types', async () => {
		...
		expect(
			screen.queryByText('Fail backup if script fails'),
