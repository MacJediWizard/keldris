...
vi.mock('../hooks/useDRRunbooks', () => ({
	...
	useCreateDRRunbook: () => ({ mutateAsync: vi.fn(), isPending: false, isError: false }),
...
describe('DRRunbooks', () => {
	...
	it('renders title', () => {
		vi.mocked(useDRRunbooks).mockReturnValue({ data: [], isLoading: false, isError: false } as ReturnType<typeof useDRRunbooks>);
		renderPage();
		expect(screen.getByText('DR Runbooks')).toBeInTheDocument();
	});

	it('shows loading state', () => {
		vi.mocked(useDRRunbooks).mockReturnValue({ data: undefined, isLoading: true, isError: false } as ReturnType<typeof useDRRunbooks>);
		renderPage();
	...
	it('renders runbooks', () => {
		vi.mocked(useDRRunbooks).mockReturnValue({
			data: [
				{ id: 'rb1', name: 'Disaster Recovery Plan', status: 'active', description: 'Main DR plan', steps: [], created_at: '2024-01-01T00:00:00Z', updated_at: '2024-01-01T00:00:00Z' },
			],
			isLoading: false,
			isError: false,
	...
	it('shows error state', () => {
		vi.mocked(useDRRunbooks).mockReturnValue({ data: undefined, isLoading: false, isError: true } as ReturnType<typeof useDRRunbooks>);
		renderPage();
