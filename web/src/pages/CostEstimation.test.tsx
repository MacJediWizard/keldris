...
vi.mock('../hooks/useCostEstimation', () => ({
	...
	useCreateCostAlert: () => ({ mutateAsync: vi.fn(), isPending: false, isError: false }),
	useDeleteCostAlert: () => ({ mutateAsync: vi.fn(), isPending: false }),
	useUpdateCostAlert: () => ({ mutateAsync: vi.fn(), isPending: false }),
}));

import { useCostSummary, useCostForecast, useCostAlerts } from '../hooks/useCostEstimation';
...
describe('CostEstimation', () => {
	...
	it('renders title', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows subtitle', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		renderPage();
		expect(screen.getByText('Monitor and forecast your cloud storage costs')).toBeInTheDocument();
	});

	it('shows loading state', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: true } as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('renders cost summary cards', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 150.50, total_yearly_cost: 1806, total_storage_size_gb: 5.0, repository_count: 3, by_type: {}, repositories: [] },
			isLoading: false,
	...
	it('shows cost summary values', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 250.75, total_yearly_cost: 3009, total_storage_size_gb: 12.5, repository_count: 5, by_type: { s3: 200, local: 50.75 }, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows storage size', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 100, total_yearly_cost: 1200, total_storage_size_gb: 8.3, repository_count: 2, by_type: {}, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows repository count card', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 100, total_yearly_cost: 1200, total_storage_size_gb: 5, repository_count: 4, by_type: {}, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows cost by storage type section', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 150, total_yearly_cost: 1800, total_storage_size_gb: 10, repository_count: 2, by_type: { s3: 100, local: 50 }, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows cost per repository section', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 100, total_yearly_cost: 1200, total_storage_size_gb: 5, repository_count: 1, by_type: {}, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows no repositories message', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 0, total_yearly_cost: 0, total_storage_size_gb: 0, repository_count: 0, by_type: {}, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
		expect(screen.getByText('No repositories with cost data')).toBeInTheDocument();
	});

	it('shows repository table with data', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: {
				total_monthly_cost: 150, total_yearly_cost: 1800, total_storage_size_gb: 10, repository_count: 1,
				by_type: { s3: 150 },
				repositories: [{ repository_id: 'r1', repository_name: 'Primary S3', repository_type: 's3', monthly_cost: 150, yearly_cost: 1800, storage_size_bytes: 10737418240, cost_per_gb: 15 }],
			},
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows repository table headers', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: {
				total_monthly_cost: 100, total_yearly_cost: 1200, total_storage_size_gb: 5, repository_count: 1,
				by_type: {},
				repositories: [{ repository_id: 'r1', repository_name: 'Test', repository_type: 'local', monthly_cost: 100, yearly_cost: 1200, storage_size_bytes: 5368709120, cost_per_gb: 20 }],
			},
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows forecast loading state', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		vi.mocked(useCostForecast).mockReturnValue({ data: undefined, isLoading: true } as ReturnType<typeof useCostForecast>);
		renderPage();
	...
	it('shows forecast empty state', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		vi.mocked(useCostForecast).mockReturnValue({ data: { forecasts: [], current_monthly_cost: 0, monthly_growth_rate: 0 }, isLoading: false } as ReturnType<typeof useCostForecast>);
		renderPage();
	...
	it('shows cost alerts section with no alerts', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		vi.mocked(useCostAlerts).mockReturnValue({ data: [], isLoading: false } as ReturnType<typeof useCostAlerts>);
		renderPage();
	...
	it('shows cost alerts when configured', () => {
		...
		vi.mocked(useCostAlerts).mockReturnValue({
			data: [{ id: 'a1', name: 'Budget Alert', monthly_threshold: 100, enabled: true, notify_on_exceed: true, notify_on_forecast: false, forecast_months: 3 }],
			isLoading: false,
	...
	it('shows disabled alert badge', () => {
		vi.mocked(useCostSummary).mockReturnValue({ data: undefined, isLoading: false } as ReturnType<typeof useCostSummary>);
		vi.mocked(useCostAlerts).mockReturnValue({
			data: [{ id: 'a1', name: 'Paused Alert', monthly_threshold: 200, enabled: false, notify_on_exceed: true, notify_on_forecast: false, forecast_months: 3 }],
			isLoading: false,
		} as ReturnType<typeof useCostAlerts>);
		renderPage();
	...
	it('shows card subtitles', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 100, total_yearly_cost: 1200, total_storage_size_gb: 5, repository_count: 2, by_type: {}, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
	...
	it('shows type badges in cost breakdown', () => {
		vi.mocked(useCostSummary).mockReturnValue({
			data: { total_monthly_cost: 150, total_yearly_cost: 1800, total_storage_size_gb: 10, repository_count: 2, by_type: { s3: 100 }, repositories: [] },
			isLoading: false,
		} as ReturnType<typeof useCostSummary>);
		renderPage();
