...
vi.mock('../hooks/useAgentGroups', () => ({
	...
	useCreateAgentGroup: () => ({ mutateAsync: vi.fn(), isPending: false, isError: false }),
	useUpdateAgentGroup: () => ({ mutateAsync: vi.fn(), isPending: false, isError: false }),
...
describe('AgentGroups', () => {
	...
	it('renders title', () => {
		vi.mocked(useAgentGroups).mockReturnValue({ data: [], isLoading: false, isError: false } as ReturnType<typeof useAgentGroups>);
		renderPage();
		expect(screen.getByText('Agent Groups')).toBeInTheDocument();
	});

	it('shows loading state', () => {
		vi.mocked(useAgentGroups).mockReturnValue({ data: undefined, isLoading: true, isError: false } as ReturnType<typeof useAgentGroups>);
		renderPage();
	...
	it('renders groups', () => {
		vi.mocked(useAgentGroups).mockReturnValue({
			data: [
				{ id: 'g1', name: 'Production', description: 'Prod servers', color: '#ef4444', member_count: 3, created_at: '2024-01-01T00:00:00Z' },
			],
			isLoading: false,
			isError: false,
	...
	it('shows error state', () => {
		vi.mocked(useAgentGroups).mockReturnValue({ data: undefined, isLoading: false, isError: true } as ReturnType<typeof useAgentGroups>);
		renderPage();
