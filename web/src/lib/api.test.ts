...
describe('agentsApi', () => {
	...
	it('rotateApiKey sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agents/a1/apikey/rotate',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('getHealthHistory passes limit param', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agents/a1/health-history?limit=50',
			expect.any(Object),
		);
	});
...
describe('agentGroupsApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'g1' });
		const { agentGroupsApi } = await import('./api');
		await agentGroupsApi.create({
			name: 'Group 1',
			description: 'desc',
		} as Parameters<typeof agentGroupsApi.create>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agent-groups',
			expect.objectContaining({ method: 'POST' }),
		);
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'g1' });
		const { agentGroupsApi } = await import('./api');
		await agentGroupsApi.update('g1', { name: 'Updated' } as Parameters<
			typeof agentGroupsApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agent-groups/g1',
	...
	it('addAgent sends POST', async () => {
		const fetchFn = mockFetch({ message: 'ok' });
		const { agentGroupsApi } = await import('./api');
		await agentGroupsApi.addAgent('g1', { agent_id: 'a1' } as Parameters<
			typeof agentGroupsApi.addAgent
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agent-groups/g1/agents',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('removeAgent sends DELETE', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/agent-groups/g1/agents/a1',
			expect.objectContaining({ method: 'DELETE' }),
		);
	});
});
...
describe('repositoriesApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'r1' });
		const { repositoriesApi } = await import('./api');
		await repositoriesApi.create({ name: 'repo' } as Parameters<
			typeof repositoriesApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories',
			expect.objectContaining({ method: 'POST' }),
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'r1' });
		const { repositoriesApi } = await import('./api');
		await repositoriesApi.update('r1', { name: 'updated' } as Parameters<
			typeof repositoriesApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/r1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('testConnection sends POST', async () => {
		const fetchFn = mockFetch({ success: true });
		const { repositoriesApi } = await import('./api');
		await repositoriesApi.testConnection({ type: 's3' } as Parameters<
			typeof repositoriesApi.testConnection
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/test-connection',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('recoverKey calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/r1/key/recover',
			expect.any(Object),
		);
	});
});
...
describe('schedulesApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 's1' });
		const { schedulesApi } = await import('./api');
		await schedulesApi.create({ name: 'sched' } as Parameters<
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 's1' });
		const { schedulesApi } = await import('./api');
		await schedulesApi.update('s1', { name: 'upd' } as Parameters<
			typeof schedulesApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/schedules/s1',
			expect.objectContaining({ method: 'PUT' }),
...
describe('policiesApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'p1' });
		const { policiesApi } = await import('./api');
		await policiesApi.create({ name: 'pol' } as Parameters<
			typeof policiesApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/policies',
			expect.objectContaining({ method: 'POST' }),
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'p1' });
		const { policiesApi } = await import('./api');
		await policiesApi.update('p1', { name: 'u' } as Parameters<
			typeof policiesApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/policies/p1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('apply sends POST', async () => {
		...
		const { policiesApi } = await import('./api');
		await policiesApi.apply('p1', { agent_ids: ['a1'] } as Parameters<
			typeof policiesApi.apply
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/policies/p1/apply',
			expect.objectContaining({ method: 'POST' }),
		);
	});
});
...
describe('backupScriptsApi', () => {
	...
	it('get calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/schedules/s1/scripts/bs1',
			expect.any(Object),
		);
	});
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'bs1' });
		const { backupScriptsApi } = await import('./api');
		await backupScriptsApi.create('s1', { name: 'script' } as Parameters<
			typeof backupScriptsApi.create
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/schedules/s1/scripts',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'bs1' });
		const { backupScriptsApi } = await import('./api');
		await backupScriptsApi.update('s1', 'bs1', { name: 'u' } as Parameters<
			typeof backupScriptsApi.update
		>[2]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/schedules/s1/scripts/bs1',
			expect.objectContaining({ method: 'PUT' }),
...
describe('snapshotCommentsApi', () => {
	...
	it('create sends POST', async () => {
		...
		const { snapshotCommentsApi } = await import('./api');
		await snapshotCommentsApi.create('sn1', { content: 'hello' } as Parameters<
			typeof snapshotCommentsApi.create
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/snapshots/sn1/comments',
			expect.objectContaining({ method: 'POST' }),
		);
	});
...
describe('restoresApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'rs1' });
		const { restoresApi } = await import('./api');
		await restoresApi.create({ snapshot_id: 'sn1' } as Parameters<
			typeof restoresApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/restores',
			expect.objectContaining({ method: 'POST' }),
...
describe('fileHistoryApi', () => {
	it('getHistory builds correct query params', async () => {
		...
		await fileHistoryApi.getHistory({
			path: '/etc/config',
			agent_id: 'a1',
			repository_id: 'r1',
		});
...
describe('alertsApi', () => {
	...
	it('acknowledge sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/alerts/al1/actions/acknowledge',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('resolve sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/alerts/al1/actions/resolve',
			expect.objectContaining({ method: 'POST' }),
		);
	});
});
...
describe('alertRulesApi', () => {
	...
	it('create sends POST', async () => {
		...
		const { alertRulesApi } = await import('./api');
		await alertRulesApi.create({ name: 'rule' } as Parameters<
			typeof alertRulesApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/alert-rules',
			expect.objectContaining({ method: 'POST' }),
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'ar1' });
		const { alertRulesApi } = await import('./api');
		await alertRulesApi.update('ar1', { name: 'u' } as Parameters<
			typeof alertRulesApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/alert-rules/ar1',
			expect.objectContaining({ method: 'PUT' }),
		);
	});
...
describe('organizationsApi', () => {
	...
	it('create sends POST', async () => {
		...
		const { organizationsApi } = await import('./api');
		await organizationsApi.create({ name: 'Org', slug: 'org' } as Parameters<
			typeof organizationsApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ organization: {} });
		const { organizationsApi } = await import('./api');
		await organizationsApi.update('o1', { name: 'U' } as Parameters<
			typeof organizationsApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('switch sends POST', async () => {
		const fetchFn = mockFetch({ organization: {} });
		const { organizationsApi } = await import('./api');
		await organizationsApi.switch({ org_id: 'o2' } as Parameters<
			typeof organizationsApi.switch
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
	...
	it('updateMember sends PUT', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/members/u1',
			expect.objectContaining({ method: 'PUT' }),
		);
	});
	...
	it('createInvitation sends POST', async () => {
		...
		const { organizationsApi } = await import('./api');
		await organizationsApi.createInvitation('o1', {
			email: 'a@b.com',
			role: 'member',
		});
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/invitations',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('deleteInvitation sends DELETE', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/invitations/i1',
			expect.objectContaining({ method: 'DELETE' }),
		);
	});
...
describe('notificationsApi', () => {
	...
	it('getChannel calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/notifications/channels/ch1',
			expect.any(Object),
		);
	});
	...
	it('createChannel sends POST', async () => {
		...
		const { notificationsApi } = await import('./api');
		await notificationsApi.createChannel({ type: 'email' } as Parameters<
			typeof notificationsApi.createChannel
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
	...
	it('updateChannel sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'ch1' });
		const { notificationsApi } = await import('./api');
		await notificationsApi.updateChannel('ch1', {
			enabled: false,
		} as Parameters<typeof notificationsApi.updateChannel>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/notifications/channels/ch1',
			expect.objectContaining({ method: 'PUT' }),
		);
	...
	it('createPreference sends POST', async () => {
		...
		const { notificationsApi } = await import('./api');
		await notificationsApi.createPreference({ channel_id: 'ch1' } as Parameters<
			typeof notificationsApi.createPreference
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
	...
	it('updatePreference sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'pr1' });
		const { notificationsApi } = await import('./api');
		await notificationsApi.updatePreference('pr1', {
			enabled: true,
		} as Parameters<typeof notificationsApi.updatePreference>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/notifications/preferences/pr1',
			expect.objectContaining({ method: 'PUT' }),
		);
	});
...
describe('auditLogsApi', () => {
	...
	it('exportCsv throws on error', async () => {
		...
		const { auditLogsApi } = await import('./api');
		await expect(auditLogsApi.exportCsv()).rejects.toThrow(
	...
	it('exportJson throws on error', async () => {
		mockFetchBlob(500, false);
		const { auditLogsApi } = await import('./api');
		await expect(auditLogsApi.exportJson()).rejects.toThrow(
			'Failed to export audit logs',
		);
	});
});
...
describe('statsApi', () => {
	...
	it('getRepositoryGrowth passes days param', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/stats/repositories/r1/growth?days=14',
			expect.any(Object),
		);
	});
	...
	it('getRepositoryHistory passes limit param', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/stats/repositories/r1/history?limit=10',
			expect.any(Object),
		);
	});
});
...
describe('verificationsApi', () => {
	...
	it('getStatus calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/r1/verification-status',
			expect.any(Object),
		);
	});
	...
	it('trigger sends POST', async () => {
		const fetchFn = mockFetch({ id: 'v1' });
		const { verificationsApi } = await import('./api');
		await verificationsApi.trigger('r1', { type: 'full' } as Parameters<
			typeof verificationsApi.trigger
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/r1/verifications',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('createSchedule sends POST', async () => {
		...
		const { verificationsApi } = await import('./api');
		await verificationsApi.createSchedule('r1', {
			cron: '0 0 * * *',
		} as Parameters<typeof verificationsApi.createSchedule>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/repositories/r1/verification-schedules',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('getSchedule calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/verification-schedules/vs1',
			expect.any(Object),
		);
	});
	...
	it('updateSchedule sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'vs1' });
		const { verificationsApi } = await import('./api');
		await verificationsApi.updateSchedule('vs1', {
			cron: '0 1 * * *',
		} as Parameters<typeof verificationsApi.updateSchedule>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/verification-schedules/vs1',
			expect.objectContaining({ method: 'PUT' }),
		);
...
describe('ssoGroupMappingsApi', () => {
	...
	it('create sends POST', async () => {
		...
		const { ssoGroupMappingsApi } = await import('./api');
		await ssoGroupMappingsApi.create('o1', {
			oidc_group_name: 'eng',
			role: 'member',
		});
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/sso-group-mappings',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('update sends PUT', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/sso-group-mappings/sm1',
			expect.objectContaining({ method: 'PUT' }),
		);
	});
	...
	it('getSettings calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/organizations/o1/sso-settings',
			expect.any(Object),
		);
	});
...
describe('maintenanceApi', () => {
	...
	it('get calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/maintenance-windows/mw1',
			expect.any(Object),
		);
	});
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'mw1' });
		const { maintenanceApi } = await import('./api');
		await maintenanceApi.create({ name: 'window' } as Parameters<
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'mw1' });
		const { maintenanceApi } = await import('./api');
		await maintenanceApi.update('mw1', { name: 'u' } as Parameters<
			typeof maintenanceApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/maintenance-windows/mw1',
			expect.objectContaining({ method: 'PUT' }),
...
describe('excludePatternsApi', () => {
	...
	it('list with category adds query param', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			expect.stringContaining('category=logs'),
			expect.any(Object),
		);
	});
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'ep1' });
		const { excludePatternsApi } = await import('./api');
		await excludePatternsApi.create({ pattern: '*.log' } as Parameters<
			typeof excludePatternsApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/exclude-patterns',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'ep1' });
		const { excludePatternsApi } = await import('./api');
		await excludePatternsApi.update('ep1', { pattern: '*.tmp' } as Parameters<
			typeof excludePatternsApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/exclude-patterns/ep1',
			expect.objectContaining({ method: 'PUT' }),
...
describe('drRunbooksApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'dr1' });
		const { drRunbooksApi } = await import('./api');
		await drRunbooksApi.create({ name: 'DR Plan' } as Parameters<
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'dr1' });
		const { drRunbooksApi } = await import('./api');
		await drRunbooksApi.update('dr1', { name: 'Updated' } as Parameters<
			typeof drRunbooksApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dr-runbooks/dr1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('activate sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dr-runbooks/dr1/activate',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('archive sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dr-runbooks/dr1/archive',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('generateFromSchedule sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dr-runbooks/s1/generate',
			expect.objectContaining({ method: 'POST' }),
		);
	});
	...
	it('createTestSchedule sends POST', async () => {
		...
		const { drRunbooksApi } = await import('./api');
		await drRunbooksApi.createTestSchedule('dr1', {
			cron: '0 0 * * *',
		} as Parameters<typeof drRunbooksApi.createTestSchedule>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dr-runbooks/dr1/test-schedules',
			expect.objectContaining({ method: 'POST' }),
		);
	});
});
...
describe('drTestsApi', () => {
	...
	it('run sends POST', async () => {
		const fetchFn = mockFetch({ id: 'dt1' });
		const { drTestsApi } = await import('./api');
		await drTestsApi.run({ runbook_id: 'dr1' } as Parameters<
...
describe('tagsApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 't1' });
		const { tagsApi } = await import('./api');
		await tagsApi.create({ name: 'prod' } as Parameters<
			typeof tagsApi.create
		>[0]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/tags',
			expect.objectContaining({ method: 'POST' }),
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 't1' });
		const { tagsApi } = await import('./api');
		await tagsApi.update('t1', { name: 'staging' } as Parameters<
			typeof tagsApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/tags/t1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('setBackupTags sends POST', async () => {
		...
		const { tagsApi } = await import('./api');
		await tagsApi.setBackupTags('b1', { tag_ids: ['t1'] } as Parameters<
			typeof tagsApi.setBackupTags
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/backups/b1/tags',
			expect.objectContaining({ method: 'POST' }),
		);
	});
});
...
describe('metricsApi', () => {
	it('getDashboardStats calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dashboard-metrics/stats',
			expect.any(Object),
		);
	});
	...
	it('getBackupSuccessRates calls correct endpoint', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/dashboard-metrics/success-rates',
			expect.any(Object),
		);
	});
...
describe('reportsApi', () => {
	...
	it('createSchedule sends POST', async () => {
		const fetchFn = mockFetch({ id: 'rs1' });
		const { reportsApi } = await import('./api');
		await reportsApi.createSchedule({ name: 'report' } as Parameters<
	...
	it('updateSchedule sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'rs1' });
		const { reportsApi } = await import('./api');
		await reportsApi.updateSchedule('rs1', { name: 'u' } as Parameters<
			typeof reportsApi.updateSchedule
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/reports/schedules/rs1',
			expect.objectContaining({ method: 'PUT' }),
	...
	it('previewReport sends POST', async () => {
		...
		expect(fetchFn).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				method: 'POST',
				body: JSON.stringify({ frequency: 'weekly', timezone: 'US/Eastern' }),
			}),
		);
	});
...
describe('onboardingApi', () => {
	...
	it('completeStep sends POST', async () => {
		const fetchFn = mockFetch({ needs_onboarding: false });
		const { onboardingApi } = await import('./api');
		await onboardingApi.completeStep(
			'org' as Parameters<typeof onboardingApi.completeStep>[0],
		);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/onboarding/step/org',
			expect.objectContaining({ method: 'POST' }),
		);
	});
...
describe('pricingApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'pr1' });
		const { pricingApi } = await import('./api');
		await pricingApi.create({ name: 'tier' } as Parameters<
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'pr1' });
		const { pricingApi } = await import('./api');
		await pricingApi.update('pr1', { name: 'u' } as Parameters<
			typeof pricingApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/pricing/pr1',
			expect.objectContaining({ method: 'PUT' }),
...
describe('costAlertsApi', () => {
	...
	it('create sends POST', async () => {
		const fetchFn = mockFetch({ id: 'ca1' });
		const { costAlertsApi } = await import('./api');
		await costAlertsApi.create({ name: 'alert' } as Parameters<
	...
	it('update sends PUT', async () => {
		const fetchFn = mockFetch({ id: 'ca1' });
		const { costAlertsApi } = await import('./api');
		await costAlertsApi.update('ca1', { name: 'u' } as Parameters<
			typeof costAlertsApi.update
		>[1]);
		expect(fetchFn).toHaveBeenCalledWith(
			'/api/v1/cost-alerts/ca1',
			expect.objectContaining({ method: 'PUT' }),
