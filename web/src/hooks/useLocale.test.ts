...
describe('useLocale', () => {
	...
	it('returns language and language lists', () => {
		...
		expect(result.current.languageNames).toEqual({ en: 'English', es: 'Español', pt: 'Português' });
	});
	...
	it('formatDuration returns time.inProgress when endDate is undefined', () => {
		const { result } = renderHook(() => useLocale(), { wrapper: createWrapper() });
		expect(result.current.formatDuration('2024-01-01T00:00:00Z', undefined)).toBe('time.inProgress');
	});

	it('formatDuration formats a duration in seconds', () => {
		const { result } = renderHook(() => useLocale(), { wrapper: createWrapper() });
		const formatted = result.current.formatDuration('2024-01-01T00:00:00Z', '2024-01-01T00:00:30Z');
		expect(formatted).toContain('time.seconds');
	});

	it('formatDuration formats a duration in minutes', () => {
		const { result } = renderHook(() => useLocale(), { wrapper: createWrapper() });
		const formatted = result.current.formatDuration('2024-01-01T00:00:00Z', '2024-01-01T00:05:30Z');
		expect(formatted).toContain('time.minutes');
	});

	it('formatDuration formats a duration in hours', () => {
		const { result } = renderHook(() => useLocale(), { wrapper: createWrapper() });
		const formatted = result.current.formatDuration('2024-01-01T00:00:00Z', '2024-01-01T02:30:00Z');
