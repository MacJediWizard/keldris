...
describe('usePolicy', () => {
	...
	it('fetches a policy', async () => {
		...
		const { result } = renderHook(() => usePolicy('p1'), {
			wrapper: createWrapper(),
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('usePolicySchedules', () => {
	...
	it('fetches schedules for a policy', async () => {
		...
		const { result } = renderHook(() => usePolicySchedules('p1'), {
			wrapper: createWrapper(),
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useCreatePolicy', () => {
	...
	it('creates a policy', async () => {
		...
		result.current.mutate({ name: 'New Policy' } as Parameters<
...
describe('useApplyPolicy', () => {
	...
	it('applies a policy', async () => {
		...
		const { result } = renderHook(() => useApplyPolicy(), {
			wrapper: createWrapper(),
		});
