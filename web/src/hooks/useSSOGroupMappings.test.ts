...
describe('useSSOGroupMappings', () => {
	...
	it('fetches mappings', async () => {
		...
		const { result } = renderHook(() => useSSOGroupMappings('o1'), {
...
describe('useSSOGroupMapping', () => {
	...
	it('fetches a mapping', async () => {
		...
		const { result } = renderHook(() => useSSOGroupMapping('o1', 'm1'), {
			wrapper: createWrapper(),
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useCreateSSOGroupMapping', () => {
	...
	it('creates a mapping', async () => {
		...
		const { result } = renderHook(() => useCreateSSOGroupMapping(), {
			wrapper: createWrapper(),
		});
		result.current.mutate({
			orgId: 'o1',
			data: { sso_group: 'admins', role: 'admin' } as Parameters<
				typeof ssoGroupMappingsApi.create
			>[1],
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useDeleteSSOGroupMapping', () => {
	...
	it('deletes a mapping', async () => {
		vi.mocked(ssoGroupMappingsApi.delete).mockResolvedValue({
			message: 'Deleted',
		});
		const { result } = renderHook(() => useDeleteSSOGroupMapping(), {
			wrapper: createWrapper(),
		});
...
describe('useSSOSettings', () => {
	...
	it('fetches SSO settings', async () => {
		vi.mocked(ssoGroupMappingsApi.getSettings).mockResolvedValue({
			enabled: true,
		});
		const { result } = renderHook(() => useSSOSettings('o1'), {
			wrapper: createWrapper(),
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useUpdateSSOSettings', () => {
	...
	it('updates SSO settings', async () => {
		vi.mocked(ssoGroupMappingsApi.updateSettings).mockResolvedValue({
			enabled: false,
		});
		const { result } = renderHook(() => useUpdateSSOSettings(), {
			wrapper: createWrapper(),
		});
		result.current.mutate({
			orgId: 'o1',
			data: { auto_provision: true } as Parameters<
				typeof ssoGroupMappingsApi.updateSettings
			>[1],
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useUserSSOGroups', () => {
	...
	it('fetches user SSO groups', async () => {
		vi.mocked(ssoGroupMappingsApi.getUserSSOGroups).mockResolvedValue({
			groups: [],
		});
		const { result } = renderHook(() => useUserSSOGroups('u1'), {
			wrapper: createWrapper(),
		});
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
