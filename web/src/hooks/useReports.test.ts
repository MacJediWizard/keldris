...
describe('useReportSchedule', () => {
	...
	it('fetches a schedule', async () => {
		...
		const { result } = renderHook(() => useReportSchedule('rs1'), { wrapper: createWrapper() });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
...
describe('useCreateReportSchedule', () => {
	...
	it('creates a schedule', async () => {
		...
		const { result } = renderHook(() => useCreateReportSchedule(), { wrapper: createWrapper() });
		result.current.mutate({ name: 'Weekly Report' } as Parameters<typeof reportsApi.createSchedule>[0]);
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
});
...
describe('useDeleteReportSchedule', () => {
	...
	it('deletes a schedule', async () => {
		vi.mocked(reportsApi.deleteSchedule).mockResolvedValue({ message: 'Deleted' });
		const { result } = renderHook(() => useDeleteReportSchedule(), { wrapper: createWrapper() });
...
describe('useSendReport', () => {
	...
	it('sends a report', async () => {
		...
		const { result } = renderHook(() => useSendReport(), { wrapper: createWrapper() });
...
describe('usePreviewReport', () => {
	...
	it('previews a report', async () => {
		vi.mocked(reportsApi.previewReport).mockResolvedValue({ html: '<p>Report</p>' });
		const { result } = renderHook(() => usePreviewReport(), { wrapper: createWrapper() });
		result.current.mutate({ frequency: 'weekly' as Parameters<typeof reportsApi.previewReport>[0] });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
});
...
describe('useReportHistory', () => {
	...
	it('fetches history', async () => {
		...
		const { result } = renderHook(() => useReportHistory(), { wrapper: createWrapper() });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
});
...
describe('useReportHistoryEntry', () => {
	...
	it('fetches a history entry', async () => {
		...
		const { result } = renderHook(() => useReportHistoryEntry('rh1'), { wrapper: createWrapper() });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
	});
